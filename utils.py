...
if __name__ == '__main__':
    data = pd.read_csv(r"F:\experiment_6\Zeisel\d40_0625\Zeisel_top2000.csv", index_col=0).T
    cells = data.index.values
    genes = data.columns.values
    data_zero = impute_dropout(data,drop_rate=0.4)
    pd.DataFrame(data_zero.T, index=genes, columns=cells).to_csv(r"F:\experiment_6\Romanov\d40\Romanov_d40.csv")